...
- (void)viewDidLoad {
    ...
    _lgView = [[LoginAgainView alloc]init];
    self.view = _lgView;
    
    [_lgView.registBtn addTarget:self action:@selector(registAction) forControlEvents:UIControlEventTouchUpInside];
    
    [_lgView.lgBtn addTarget:self action:@selector(loginAction) forControlEvents:UIControlEventTouchUpInside];
    
    [_lgView.forgetPwdBtn addTarget:self action:@selector(resetPwdAction) forControlEvents:UIControlEventTouchUpInside];
    
}

//注册
- (void)registAction{
    [self.navigationController pushViewController:[RegistController new] animated:NO];
}

//忘记密码
- (void)resetPwdAction{
    [self.navigationController pushViewController:[ResetPwdController new] animated:NO];
}

//登陆
- (void)loginAction{
    [AVUser logInWithUsernameInBackground:_lgView.idField.text password:_lgView.pwdTxtField.text block:^(AVUser *user, NSError *error) {
        if (user != nil) {
            UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"提示" message:@"登陆完成,即将跳转页面" preferredStyle:UIAlertControllerStyleAlert];
            UIAlertAction *okAct = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
