...
- (void)viewDidLoad {
    ...
    [_registView.registBtn addTarget:self action:@selector(registAction) forControlEvents:UIControlEventTouchUpInside];
}

- (void)registAction{
    AVUser *user = [AVUser user];
    user.username = self.registView.idTxtField.text;
    user.password = self.registView.pwdTxtField.text;
    user.email = self.registView.mailTxtField.text;
    user.mobilePhoneNumber = self.registView.phoneTxtField.text;
    
    [user signUpInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
        if (succeeded) {
            [self p_showAlertView:@"提示" message:@"注册成功,即将跳转页面"];
        } else {
